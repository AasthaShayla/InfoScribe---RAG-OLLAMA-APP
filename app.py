...
- 30% True/False with explanations
- 20% Short answer (1-2 sentences)
...
def generate_quiz_questions(content: str, num_questions: int = 5):
    """Generate quiz questions from content - optimized and reliable"""
    return create_smart_quiz(content, num_questions)

def create_smart_quiz(content: str, num_questions: int = 5):
    """Create a comprehensive quiz with different question types"""
    questions = []
    ...
    for i in range(min(num_questions, len(sentences))):
        ...
        if question_type == 'multiple_choice':
            # Create multiple choice from sentence
            words = [w.strip('.,!?:;') for w in sentence.split() if len(w) > 4 and w.isalpha()]
            if len(words) > 0:
                key_word = random.choice(words)
                
                # Create wrong options based on content
                all_words = [w.strip('.,!?:;') for w in content.split() if len(w) > 4 and w.isalpha()]
                wrong_options = random.sample([w for w in set(all_words) if w != key_word][:10], min(3, len(set(all_words))-1))
                
                # Fallback wrong options
                if len(wrong_options) < 3:
                    fallback_options = ["concept", "process", "method", "system", "approach", "technique"]
                    wrong_options.extend(random.sample(fallback_options, 3 - len(wrong_options)))
                ...
                questions.append({
                    ...
                    "question": f"Based on the context: '{sentence[:100]}...', which term is most relevant?",
                    "options": options,
                    "correct_answer": key_word,
                    "explanation": f"The term '{key_word}' appears in the original context and is most relevant.",
                    "difficulty": "Medium"
                })
            ...
            questions.append({
                ...
                "correct_answer": "True",
                "explanation": "This statement is taken directly from the document content.",
                "difficulty": "Easy"
            })
            ...
            if len(words) > 8:
                ...
                questions.append({
                    "type": "short_answer",
                    "question": f"Complete this sentence: '{key_phrase}...' What comes next?",
                    "correct_answer": " ".join(words[4:8]),
                    "explanation": f"The continuation is: '{' '.join(words[4:8])}'",
                    "difficulty": "Medium"
                })
            ...
            if len(words) > 5:
                ...
                blank_word = words[blank_index].strip('.,!?:;')
                if len(blank_word) > 3:
                    question_text = sentence.replace(words[blank_index], "______", 1)
...
def create_fallback_quiz(content: str, num_questions: int = 3):
    """Simple fallback quiz - kept for compatibility"""
    return create_smart_quiz(content, num_questions)
    ...
    if st.session_state.quiz_state['active_quiz'] is None:
        ...
        if current_q < len(questions):
            ...
            if question['type'] == 'multiple_choice' and 'options' in question:
                ...
                user_answer = st.radio("Select your answer:", ["True", "False"], key=f"q_{current_q}")
            ...
            if submit_clicked and user_answer:
                ...
                if question['type'] == 'multiple_choice':
                    ...
                    correct = user_clean in correct_clean or correct_clean in user_clean
